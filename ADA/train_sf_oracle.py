...
def train(cfg, args, run_name='',ckpt_save_dir = '',tb_dir = ''):
    ...
    for iteration in tqdm(range(start_iteration, max_iteration)):
        ...
        if cfg.TRAIN.XMUDA.lambda_ce_trg > 0:
            # oracle: target domain supervised with 100% labels

            seg_loss_trg_2d = cfg.TRAIN.XMUDA.lambda_ce_trg * F.cross_entropy(preds_2d['seg_logit'], data_batch_trg['seg_label'], weight=class_weights)
            seg_loss_trg_3d = cfg.TRAIN.XMUDA.lambda_ce_trg * F.cross_entropy(preds_3d['seg_logit'], data_batch_trg['seg_label'], weight=class_weights)
