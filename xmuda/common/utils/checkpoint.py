...
class CheckpointerV2(Checkpointer):
    ...
    def load_deal_dual(self,model_cfg,path=None,resume=True, resume_states=True,load_4_backbone = False):
        if resume and self.has_checkpoint():
            # override argument with existing checkpoint
            path = self.get_checkpoint_file()
        if not path:
        ...
        weights = checkpoint.pop('model')
        if model_cfg.DUAL_HEAD and 'linear2.weight' not in weights:
            self._print('Expanding single head model to dual head')
            weights['linear2.weight'] = weights['linear.weight']
            weights['linear2.bias'] = weights['linear.bias']
        elif not model_cfg.DUAL_HEAD and 'linear2.weight' in weights:
            self._print('Ignoring extra output head')
        ...
        if isinstance(self.model, (DataParallel, DistributedDataParallel)):
            ...
            if load_4_backbone:
                self.model.backbone.load_state_dict(weights)
            else:
