...
class SemanticKITTIBase(Dataset):
    ...
    def __init__(self,
        ...
        if pselab_paths:
            ...
            pseudo_label_2d = np.concatenate([data['pseudo_label_2d'] for data in self.pselab_data]).astype(int)
            # ori_pseudo_label_2d = copy.deepcopy(pseudo_label_2d)
            if not no_refine:
            ...
            if self.pselab_data[0]['probs_3d'] is not None:
                ...
                pseudo_label_3d = np.concatenate([data['pseudo_label_3d'] for data in self.pselab_data]).astype(int)
                # ori_pseudo_label_3d = copy.deepcopy(pseudo_label_3d)
                if not no_refine:
                    pseudo_label_3d = refine_pseudo_labels(probs3d, pseudo_label_3d)
            else:
                pseudo_label_3d = None

            if self.pselab_data[0]['pseudo_label_ensemble'] is not None:
                pseudo_label_ensemble = np.concatenate([data['pseudo_label_ensemble'] for data in self.pselab_data]).astype(int)
            else:
            ...
            for data_idx in range(len(self.pselab_data)):
                ...
                # self.pselab_data[data_idx]['ori_pseudo_label_2d'] = ori_pseudo_label_2d[left_idx:right_idx]

                if pseudo_label_3d is not None:
                    self.pselab_data[data_idx]['pseudo_label_3d'] = pseudo_label_3d[left_idx:right_idx]
                    # self.pselab_data[data_idx]['ori_pseudo_label_3d'] = ori_pseudo_label_3d[left_idx:right_idx]
                else:
                    self.pselab_data[data_idx]['pseudo_label_3d'] = None
                
                if pseudo_label_ensemble is not None:
        ...
        if label_mask_path:
            print('Load label mask ', label_mask_path)
            self.label_mask = []
            self.label_mask.extend(np.load(label_mask_path, allow_pickle=True))
...
class SemanticKITTISCN(SemanticKITTIBase):
    def __init__(self,
        ...
        self.return_ori_points = return_ori_points
        self.new_pselab = new_pselab
        print('self.return_ori_points:',self.return_ori_points)
        print('self.new_pselab:',self.new_pselab)
        self.shape_set = set()
        self.shape_set_size = 0
        self.ref_shape = None
        # pdb.set_trace()
    ...
    def __getitem__(self, index):
        ...
        if self.crop_size:
            ...
            for _ in range(10):
                if self.bottom_crop:
                    ...
                    # Random cropping may result in inconsistent numbers of points being cropped
                    # Eliminate random differences in mask lengths generated by each dataloader
                    left = int(math.floor(0.5 * (image.size[0] + 1 - self.crop_size[0])))
        ...
        #     self.ref_shape = image.size
        #     print(f'##### img idx {index} size {self.ref_shape}.')
        # if image.size != self.ref_shape:
        #     # print(f'#####resize img idx {index} to size {self.ref_shape}.')
        #     image = image.resize(self.ref_shape, Image.BILINEAR)
        ...
        self.shape_set.add(out_dict['img'].shape)
        if len(self.shape_set) != self.shape_set_size:
            print(self.shape_set)
            print(img_path)
            self.shape_set_size = len(self.shape_set)
        
        # print(out_dict['img'].shape)
        ...
        if seg_label is not None:
            ...
            if index == 0:
                print('len(out_dict["seg_label"]) idx 0: ',len(out_dict['seg_label']))
        out_dict['img_indices'] = out_dict['img_indices'][idxs]

        if self.pselab_data is not None:
            if not self.new_pselab:
                out_dict['pseudo_label_2d'] = self.pselab_data[index]['pseudo_label_2d'][keep_idx][idxs]
                # out_dict['ori_pseudo_label_2d'] = self.pselab_data[index]['ori_pseudo_label_2d'][keep_idx][idxs]
                if self.pselab_data[index]['pseudo_label_3d'] is None:
                    ...
                    # out_dict['ori_pseudo_label_3d'] = self.pselab_data[index]['ori_pseudo_label_3d'][keep_idx][idxs]

                if self.pselab_data[index]['pseudo_label_ensemble'] is None:
                ...
                out_dict['pseudo_label_3d'] = self.pselab_data[index]['pseudo_label_3d']
                out_dict['pseudo_label_ensemble'] = self.pselab_data[index]['pseudo_label_ensemble']
                
        if self.label_mask is not None:
            # print(len(self.label_mask))
            # print(index)
            out_dict['label_mask'] = self.label_mask[index]
        ...
        if self.return_ori_points:
            out_dict['ori_points'] = points[idxs]
        return out_dict
