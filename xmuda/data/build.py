...
def build_dataloader(cfg, mode='train', domain='source', start_iteration=0, different_batch_size=None,batch_base = False):
    ...
    if domain == 'target' and (not is_train or mode == 'train_labeled'):
        print(f"domain:{domain},is_train:{is_train},mode:{mode}")
        print("domain == 'target' and (not is_train or mode == 'train_labeled')")
        print("dataset_kwargs.pop('pselab_paths')")
        dataset_kwargs.pop('pselab_paths')
    if ('pselab_paths' in dataset_kwargs.keys()) and ('label_mask_path' in dataset_kwargs.keys()) and (len(dataset_kwargs['label_mask_path'])== 0):
        # 当ADA设置下label mask尚未准备好时，不加载伪标签。
        print(f"dataset_kwargs.keys():{dataset_kwargs.keys()},len(dataset_kwargs['label_mask_path']):{len(dataset_kwargs['label_mask_path'])}")

        print("'label_mask_path' in dataset_kwargs.keys() and len(dataset_kwargs['label_mask_path'])== 0")
        
        print("dataset_kwargs.pop('pselab_paths')")
        dataset_kwargs.pop('pselab_paths')
        
    # if domain == 'target' and (not is_train or mode == 'train_labeled'):
    #     dataset_kwargs.pop('label_mask_path')
