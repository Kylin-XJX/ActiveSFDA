        ...
        for iteration, data_batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            for batch_ind in range(len(points_idx)):
                ...
                pseudo_label_dict = {
                    ...
                    # 'ori_probs_2d':curr_probs_2d.cpu().numpy(),
                    ...
                    # 'ori_probs_3d':curr_probs_3d.cpu().numpy() if model_3d else None,
...
def test(cfg, args, output_dir='',run_name = ''):
    ...
    logger.info(f'args.ckpt2d:{args.ckpt2d}')
    logger.info(f'args.ckpt3d:{args.ckpt3d}')
    if args.ckpt2d:
        # load weight if specified
        checkpointer_2d.load_deal_dual(cfg.MODEL_2D,args.ckpt2d, resume=False,load_4_backbone=False,resume_states=False)
    else:
    ...
    if args.ckpt3d:
        # load weight if specified
        checkpointer_3d.load_deal_dual(cfg.MODEL_3D,args.ckpt3d, resume=False,load_4_backbone=False,resume_states=False)
    else:
        # load last checkpoint
    ...
    if args.pselab:
        ...
        if args.ex_tag:
            pselab_path = osp.join(pselab_dir,run_name+cfg.DATASET_TARGET.TEST[0] + args.ex_tag + '.npy')
        else:
            pselab_path = osp.join(pselab_dir,run_name+cfg.DATASET_TARGET.TEST[0] + '.npy')
